...
-- Não altera as regras nutricionais; apenas adiciona persistência/estado para
-- gerar por etapas (blueprint semanal → dia a dia → validação → finalização).
...
-- ---------- substitution_groups (reutilizáveis por job) ----------
CREATE TABLE IF NOT EXISTS public.substitution_groups (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  job_id uuid NOT NULL REFERENCES public.plan_generation_jobs(id) ON DELETE CASCADE,
  group_key text NOT NULL,       -- ex: "cho_cafe", "ptn_almoco"
  main_macro text,               -- 'cho' | 'protein' | 'fat' | 'veg' | 'fruit'
  target_g numeric,
  options jsonb,                 -- [{ name, food_id, grams, household_measure }]
  created_at timestamptz NOT NULL DEFAULT now(),
