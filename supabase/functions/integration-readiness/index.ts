...
Deno.serve(async req=>{
 ...
 return reply({success:true,automaticPaused:true,google:{savedAuthorization:!!connection,hasRefresh:!!connection?.refresh_token,expired:!connection?.token_expires_at||Date.parse(connection.token_expires_at)<Date.now(),missingConfiguration:missing,meetEnabled:false,message:missing.length?'O Google precisa das credenciais OAuth deste sistema e de uma nova autorização.':'Autorização antiga encontrada. A renovação OAuth e a criação do Meet ainda precisam ser publicadas e verificadas.'},adminNotifications:{enabled:false,phoneSuffix:(admin?.admin_whatsapp_number||'').replace(/\D/g,'').slice(-4),message:'Avisos de check-in, consulta e anamnese não estão ativos neste projeto. O contato administrativo cadastrado será conferido antes da reativação.'}});
});
